...
const FileUploadModal: React.FC<FileUploadModalProps> = ({ isOpen, onClose, onUpload }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Extract text from each file using the same logic as PDFUploader
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file.type === 'application/pdf') {
            console.log(`Extracting text from PDF: ${uploadedFile.file.name}`);
            ...
            if (typeof extractedText === 'string' && extractedText.trim()) {
              ...
              console.log(`Extracted ${extractedText.length} characters from ${uploadedFile.file.name}`);
            }
          } else if (uploadedFile.file.type === 'text/plain') {
            const text = await uploadedFile.file.text();
            if (text.trim()) {
              contents.push(text);
              console.log(`Read ${text.length} characters from ${uploadedFile.file.name}`);
            }
          }
      ...
      // Send extracted text to the backend for analysis using the same API as PDFUploader
      const response = await apiService.analyzeContentAndSuggest(contents);
        ...
        >
          ...
          <div className="p-6">
            ...
            <div className="flex gap-3 justify-end pt-4 border-t border-gray-200">
              ...
              <button 
                className="px-6 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg hover:from-purple-700 hover:to-blue-700 font-inter disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={handleUpload}
                disabled={uploadedFiles.length === 0 || isUploading}
              >
                {isUploading ? 'Processing...' : 'Analyze & Generate Topics'}
              </button>
            </div>
          </div>
        </div>
